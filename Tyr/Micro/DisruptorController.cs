...
namespace SC2Sharp.Micro
{
    public class DisruptorController : CustomController
    {
        ...
        public override bool DetermineAction(Agent agent, Point2D target)
        {
            ...
            if (PhaseFrame.ContainsKey(agent.Unit.Tag) && Bot.Main.Frame - PhaseFrame[agent.Unit.Tag] <= 22)
                return true;
        ...
        {
            if (PhaseFrame.ContainsKey(agent.Unit.Tag) && Bot.Main.Frame - PhaseFrame[agent.Unit.Tag] <= 224)
                return false;
            ...
            foreach (Unit unit in Bot.Main.Enemies())
            {
                ...
                if (count >= 6)
                {
                    ...
                    CollectionUtil.Add(PhaseFrame, agent.Unit.Tag, Bot.Main.Frame);
                    return true;
                }
