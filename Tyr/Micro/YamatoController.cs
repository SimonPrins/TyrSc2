...
namespace Tyr.Micro
{
    public class YamatoController : CustomController
    {
        ...
        public override bool DetermineAction(Agent agent, Point2D target)
        {
            ...
            if (!Bot.Bot.Observation.Observation.RawData.Player.UpgradeIds.Contains(UpgradeType.YamatoCannon))
                return false;

            if (!YamatoFrames.ContainsKey(agent.Unit.Tag))
                YamatoFrames.Add(agent.Unit.Tag, -10000);
            if (Bot.Bot.Frame - YamatoFrames[agent.Unit.Tag] < 44)
                return true;
            if (Bot.Bot.Frame - YamatoFrames[agent.Unit.Tag] < 22.4 * 72)
                return false;
            ...
            foreach (Unit enemy in Bot.Bot.Enemies())
            {
                ...
                if (agent.DistanceSq(enemy) <= 10 * 10)
                {
                    ...
                    YamatoFrames[agent.Unit.Tag] = Bot.Bot.Frame;
                    return true;
                }
            }
