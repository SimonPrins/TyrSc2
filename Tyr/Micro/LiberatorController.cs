...
namespace Tyr.Micro
{
    public class LiberatorController : CustomController
    {
        ...
        public override bool DetermineAction(Agent agent, Point2D target)
        {
            ...
            else if (Tyr.Bot.Frame - LastEnemyFrame[agent.Unit.Tag] <= 22.4 * KeepLiberatorSiegedTime)
                closeEnemy = true;
        ...
        private Unit GetAirDefense(Agent agent)
        {
            float distance = 8 * 8;
            Unit result = null;
            foreach (Unit airDefense in Tyr.Bot.Enemies())
            {
        ...
        {
            foreach (Agent liberator in Tyr.Bot.UnitManager.Agents.Values)
            {
