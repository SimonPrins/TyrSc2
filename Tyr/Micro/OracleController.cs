...
namespace Tyr.Micro
{
    public class OracleController : CustomController
    {
        ...
        public bool EvadeMines(Agent agent, Point2D target)
        {
            ...
            {
                if (Tyr.Bot.EnemyMineManager.BurrowFrame.ContainsKey(mine.Tag) && Tyr.Bot.Frame - Tyr.Bot.EnemyMineManager.BurrowFrame[mine.Tag] <= 16)
                    continue;
            ...
            {
                bool alreadyRetreating = RetreatFrame.ContainsKey(agent.Unit.Tag) && Tyr.Bot.Frame - RetreatFrame[agent.Unit.Tag] <= 5;

                if (dist <= 7 * 7 || alreadyRetreating)
                {

                    if (RetreatFrame.ContainsKey(agent.Unit.Tag))
                        RetreatFrame[agent.Unit.Tag] = Tyr.Bot.Frame;
                    else
                        RetreatFrame.Add(agent.Unit.Tag, Tyr.Bot.Frame);
