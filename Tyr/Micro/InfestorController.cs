...
namespace Tyr.Micro
{
    public class InfestorController : CustomController
    {
        ...
        public override bool DetermineAction(Agent agent, Point2D target)
        {
            ...
            if (Bot.Main.Frame != CleanFungalTargetsFrame)
            {
                CleanFungalTargetsFrame = Bot.Main.Frame;

                List<ulong> clearTags = new List<ulong>();
                foreach (FungalTarget fungal in FungalTargets.Values)
                    if (Bot.Main.Frame - fungal.Frame >= 22.4 * 3)
            ...
            if (NeuralFrame.ContainsKey(agent.Unit.Tag) && Bot.Main.Frame - NeuralFrame[agent.Unit.Tag] < 22)
                return true;
            ...
            foreach (Unit unit in Bot.Main.Enemies())
            {
                ...
                if (SC2Util.DistanceSq(unit.Pos, agent.Unit.Pos) <= 10 * 10)
                {
                    ...
                    if (!NeuralFrame.ContainsKey(agent.Unit.Tag))
                        NeuralFrame.Add(agent.Unit.Tag, Bot.Main.Frame);
                    else NeuralFrame[agent.Unit.Tag] = Bot.Main.Frame;
        ...
        public bool Fungal(Agent agent)
        {
            ...
            if (FungalTargets.ContainsKey(agent.Unit.Tag) && Bot.Main.Frame - FungalTargets[agent.Unit.Tag].Frame <= 22)
                return true;
            ...
            foreach (UnitLocation mine in Bot.Main.EnemyMineManager.Mines)
            {
                ...
                {
                    CollectionUtil.Add(FungalTargets, agent.Unit.Tag, new FungalTarget() { Pos = SC2Util.To2D(mine.Pos), Frame = Bot.Main.Frame, InfestorTag = agent.Unit.Tag });
            ...
            foreach (Unit unit in Bot.Main.Enemies())
            {
                ...
                {
                    CollectionUtil.Add(FungalTargets, agent.Unit.Tag, new FungalTarget() { Pos = SC2Util.To2D(unit.Pos), Frame = Bot.Main.Frame, InfestorTag = agent.Unit.Tag });
