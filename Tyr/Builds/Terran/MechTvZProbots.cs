    ...
    public class MechTvZProbots : Build
    {
        ...
        public override void OnFrame(Tyr tyr)
        {
            ...
            if (tyr.EnemyStrategyAnalyzer.TotalCount(UnitTypes.MUTALISK) > 0)
                DefendMutas = true;
            if (tyr.EnemyStrategyAnalyzer.TotalCount(UnitTypes.ULTRALISK) + tyr.EnemyStrategyAnalyzer.TotalCount(UnitTypes.ULTRALISK_CAVERN) > 0)
                UltralisksDetected = true;
            if (tyr.EnemyStrategyAnalyzer.TotalCount(UnitTypes.ROACH) + tyr.EnemyStrategyAnalyzer.TotalCount(UnitTypes.HYDRALISK) >= 10)
                RoachHydra = true;
            ...
            if (!InitialAttackDone && (tyr.EnemyStrategyAnalyzer.Count(UnitTypes.ROACH) > 0
                || tyr.EnemyStrategyAnalyzer.Count(UnitTypes.HYDRALISK) >= 5
            ...
            if (tyr.EnemyStrategyAnalyzer.TotalCount(UnitTypes.BANELING) >= 10)
                DesiredMines = 12;
        ...
        public override void Produce(Tyr tyr, Agent agent)
        {
            ...
            else if (agent.Unit.UnitType == UnitTypes.BARRACKS)
            {
                if (Minerals() >= 50
                    ...
                    && (!tyr.EnemyStrategyAnalyzer.Expanded || Count(UnitTypes.BUNKER) > 0))
                    agent.Order(560);
            }
            ...
            else if (agent.Unit.UnitType == UnitTypes.FACTORY_TECH_LAB)
            {
                if (Count(UnitTypes.HELLBAT) + Count(UnitTypes.HELLION) > 0
                    && !Tyr.Bot.Observation.Observation.RawData.Player.UpgradeIds.Contains(19))
                {
                    if (Gas() >= 150
                        && Minerals() >= 150)
                        agent.Order(761);
                    ...
                    && !Tyr.Bot.Observation.Observation.RawData.Player.UpgradeIds.Contains(144))
                {
                    if (Gas() >= 150
                        && Minerals() >= 150)
                        agent.Order(769);
                    ...
                    && !Tyr.Bot.Observation.Observation.RawData.Player.UpgradeIds.Contains(122))
                {
                    if (Gas() >= 75
                        && Minerals() >= 75)
                        agent.Order(764);
