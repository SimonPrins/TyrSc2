...
namespace Tyr.Tasks
{
    public class CombatGroup
    {
        ...
        public void AttackAt(Point2D target, Task task)
        {
            ...
            Tyr.Bot.DrawText("totalResources: " + totalResources + " enemyResources: " + enemyResources);
            ...
            Tyr.Bot.DrawText("CombatState: " + State);

            foreach (Agent agent in Units)
            {
                if (agent.Unit.UnitType == UnitTypes.MEDIVAC)
                {
                    UpdateMedivacRetreatTarget(Tyr.Bot);
                ...
                else if (State == Retreat)
                {
                    ...
                    if (!close)
                        task.Attack(agent, target);
                    if (agent.Unit.WeaponCooldown < 3)
                        task.Attack(agent, SC2Util.To2D(Tyr.Bot.MapAnalyzer.StartLocation));
                    else
