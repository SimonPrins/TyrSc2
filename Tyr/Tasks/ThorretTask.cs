...
namespace Tyr.Tasks
{
    public class ThorretTask : Task
    {
        ...
        {
            Point2D startLocation = SC2Util.To2D(Bot.Main.MapAnalyzer.StartLocation);
            AddTask(SC2Util.Point(startLocation.X + 6, startLocation.Y + 6));
            AddTask(SC2Util.Point(startLocation.X - 6, startLocation.Y + 6));
            AddTask(SC2Util.Point(startLocation.X + 6, startLocation.Y - 6));
            AddTask(SC2Util.Point(startLocation.X - 6, startLocation.Y - 6));

            Tasks.Sort((a, b) => (int)(SC2Util.DistanceSq(a.IdleLocation, Bot.Main.TargetManager.PotentialEnemyStartLocations[0]) - SC2Util.DistanceSq(b.IdleLocation, Bot.Main.TargetManager.PotentialEnemyStartLocations[0])));
            int i = 0;
        ...
        public override bool DoWant(Agent agent)
        {
            return (agent.Unit.UnitType == UnitTypes.THOR_SINGLE_TARGET)
                && units.Count == 0
                && agent.DistanceSq(Bot.Main.MapAnalyzer.StartLocation) <= 55 * 55;
        }
